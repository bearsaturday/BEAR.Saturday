 ...
 * @version    SVN: Release: $Id: Handler.php 1230 2009-11-17 13:03:02Z koriyama@users.sourceforge.jp $
...
class BEAR_Resource_Server_Handler extends Net_Server_Handler
{
    ...
    public function sendData($clientId, $method, BEAR_Ro $ro)
    {
        ...
        if (isset($hearders['broadcast'])) {
            $this->_server->broadcastData($code . PHP_EOL, array(
                $clientId));
            $this->_server->broadcastData('Content-Type: text/php' . PHP_EOL, array(
                $clientId));
