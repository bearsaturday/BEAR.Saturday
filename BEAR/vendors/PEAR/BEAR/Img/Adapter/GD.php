 ...
 * @version   SVN: Release: $Id: GD.php 1309 2010-01-04 08:02:38Z koriyama@users.sourceforge.jp $
...
class BEAR_Img_Adapter_GD extends BEAR_Img_Adapter
{
    ...
    public function load($file, $format = '')
    {
        ...
        if ($format) {
            ...
            if (stristr(strtolower($this->file), '.gif')) {
                ...
                throw $this->_exception('Unknown file format', array(
                    'info' => $info));
            }
        }
        // イメージリソース作成
    ...
    public function show($format = false)
    {
        ...
        $this->_log->log('IMG show', array('format' => $format,
    ...
    public function save($filePath, $format)
    {
        ...
        $this->_log->log('IMG saved', array('format' => $format,
            'file' => $filePath,
            'rsc' => (string)$this->_imgResource,
            'result' => $result));
    }
