 ...
 * @version   SVN: Release: $Id: Emoji.php 1218 2009-11-15 03:03:20Z koriyama.sourceforge $
...
class BEAR_Emoji extends BEAR_Base
{
    ...
    {
    	$ua = (isset($this->_config['string'])) ? $this->_config['string'] : BEAR::dependency('BEAR_Agent')->getUa();
    	$string = isset($this->_config['string']) ? $this->_config['string'] : '';
    	$this->_string = $string;
    }
    ...
    public function getInstance($string, $ua = false)
    {
        ...
        self::$_ua = $agent->getUa();
        self::$_uaShortCode = $agent->agentMobile->getCarrierShortName();

        $ua = ($ua === false) ? self::$_uaShortCode : $ua;
        $id = md5($string . $ua);
     ...
     * @return string
     */
    public function makeDecEntity($string, $isSbUtfPost = false)
    {
    ...
    {
        $regex = '/&#(\d{5});/is';
        $string = preg_replace_callback($regex, array(__CLASS__,
            '_onHexEntity'), $string);
        return $string;
    }
    ...
    private static function _onEmojiImage($matches)
    {
        ...
        if ($ua !== $emojiUa && $emojiUa !== false) {
        	$emojiFontSize = ($ua === BEAR_Agent::UA_DEFAULT && isset(self::$_staticConfig['emoji_path'])
        	 &&self::$_staticConfig['pc_emoji_size'] !== 20) ? '12' : '20';
    ...
    public function removeEmoji($string)
    {
        ...
        $log['after'] = $string;
        //$log = BEAR::dependency('BEAR_Log');     $log->log('AJAX', $values); ('removeEmoji', $log);
        return $string;
    }
     ...
     * _BEAR_CONVERT_3GC_WEB_ENCODEがオンのときBEAR_Pageで$_POSTに対して使用しています
     * </pre>
     *
     * @param string $string 文字列
     *
    ...
    public function makeEmojiBinary($html)
    {
        if (($to == BEAR_Agent::UA_DOCOMO || $to == BEAR_Agent::UA_EZWEB) && preg_match('/&#(\d{5});/', $converted)) {
        $decCode = (int)preg_replace('/&#(\d{5});/', '$1', $converted);
        $len = strlen($decCode);
