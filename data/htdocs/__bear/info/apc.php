...
$vardom = array('OB' => '/^\d+$/',  // operational mode switch
...
<div class="head">
<h1 class="apc">
<div class="logo"><span class="logo"><a
	href="http://pecl.php.net/package/APC">APC</a></span></div>
            ...
            foreach ($scope_list as $j => $list) {
                foreach ($cache[$list] as $i => $entry) {
                    ...
                    foreach ($entry as $k => $value) {
                        ...
                        echo "<tr class=tr-$m>", "<td class=td-0>", ucwords(preg_replace("/_/", " ", $k)), "</td>", "<td class=td-last>", (preg_match("/time/", $k) && $value != 'None') ? date(DATE_FORMAT, $value) : $value, "</td>", "</tr>";
                        $m = 1 - $m;
                    }
                    if ($fieldkey == 'info') {
